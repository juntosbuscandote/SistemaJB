function Perfiles() {
	this.lstPerfiles = $('#lstPerfil');
	this.gMethods = system.globalMethods;
}

Perfiles.prototype.init = function() {
	this.handler();	
	this.listarPerfiles();
}
Perfiles.prototype.handler = function() {
	var obj=this;
	
	$('body').on('click', '#btn_search', function(event) {
    	obj.reloadDataTableGrupoComer();
    });
	
	if ($("#fProcess").val() != "save") {
//		$("#grupocom_vname").attr("readonly", true);
//		$("#grupocom_vdescrip").attr("readonly", true);		
		
	}
	
	$("#btnSalir").click(function(e){
		bootbox.confirm("<label class='text-danger'>Se descartarán los cambios ¿Desea Salir?</label>",function(res){
			if (res) {
				location.href = "/SistemaFE/perfiles/";
			}
		});
	});
	
	$("#btnSalirUP").click(function(e){
		bootbox.confirm("<label class='text-danger'>¿Desea Salir?</label>",function(res){
			if (res) {
				location.href = "/SistemaFE/perfiles/";
			}
		});
	});
	
	$("#btnProccess").click(function() {
		var l = Ladda.create(this);
		l.start();
		if ($("#frmPerfil").valid()) {
			if ($("#fProcess").val() == "save") {
				obj.crearPerfil(this,l);
			} else {
				obj.updatePerfil();
			}
		} else {
			l.stop();
			// alert("Verificar campos requeridos");
			bootbox.alert("<label>Verificar campos requeridos</label>");
		}
	});
	
	$("#btnProccessMod").click(function() {
		var l = Ladda.create(this);
		l.start();
		obj.asigModPerfiles(this,l);
	});
	
	$("#btnBuscarUser").click(function() {
		var l = Ladda.create(this);
		l.start();
		obj.buscarUsers(this,l);
	});
	
	
	obj.gMethods.setValidatorDefaults();
	$("#frmPerfil").validate({
		rules : {
			perfil_vname : {
				required : true
			}
		}

	});
	

	$('body').on('click', '.btnEditarPerfil', function() {
		var boton=$(this);
		var idGC=boton.attr("attr-id");
		var nombreGC=boton.attr( "attr-nombre" );
		bootbox.confirm("<label class='text-danger'>¿Esta seguro que desea modificar el Perfil</label>"+' '+nombreGC+
		"<label class='text-danger'>?</label>",function(res){
			if (res) {
				location.href="/SistemaFE/perfiles/EditarPerfil?idPerfil="+idGC;
			}
		});
	}); 

	$('body').on('click', '.btnAsigPerm', function() {
		var boton=$(this);
		var idGC=boton.attr( "attr-id" );
		var nombreGC=boton.attr( "attr-nombre" );
		bootbox.confirm("<label class='text-danger'>¿Esta seguro que desea asignar Permisos al Perfil</label>"+' '+nombreGC+
				"<label class='text-danger'>?</label>",function(res){
					if (res) {
						location.href="/SistemaFE/perfiles/AsignarPermisos?idPerfil="+idGC;
					}
				});
	});
	
		$('body').on('click', '.btnCambiarEstPerf', function() {
			var boton=$(this);
			var idGC=boton.attr( "attr-id" );
			var nombreGC=boton.attr( "attr-nombre" );
			var estadoGC=boton.attr( "attr-estado" );
			var texto="";
			var textoRpta="";
			if(estadoGC=="0"){
            	texto="Habilitar";
            	textoRpta="habilitó";
            }else{
            	texto="Inhabilitar";
            	textoRpta="inhabilitó";
            }
			
			bootbox.confirm("<label class='text-danger'>¿Esta seguro que desea "+texto+" el Grupo Comercial:</label>"+' '+nombreGC+" <label class='text-danger'>?</label>",function(res){
				if (res) {
					$.ajax({
						url : "/SistemaFE/perfiles/updatePerfil/",
						type : "POST",
						data : {
							"perfil_id":idGC,
							"estado":estadoGC
						},
						dataType : "json",
						beforeSend : function() {
						},
						success : function(data) {
							if (data.msg == "ok") {
								bootbox.alert({
					                size: null,
					                title: null,
					                message: "<label class='text-danger'>Se "+textoRpta+" el Perfil :</label>"+nombreGC,
					                callback: function () {
					                	location.href = "/SistemaFE/perfiles/";
					                }
					            });
							}
						},
						error : function(xhr, statusText, errorThrown) {
							console.log("ocurrio un error");  
						}
					});
					
				}
			});
		});
		
		$("#grupocom_vmodule").on( 'change', function() {
			var obj=$("#grupocom_vmodule").val();
		    console.log("ver :"+obj);
		    $('#grupocom_vmodule option').each(function(){
		    	console.log($(this).val());
		    	if(obj==""){
		    		$("#"+$(this).val()).show();
		    	}else{
		    		if($(this).val()==obj){
		    			$("#"+$(this).val()).show();
		    		}else{
		    			$("#"+$(this).val()).hide();
		    		}
		    	}
		    	});
		});
		
}

Perfiles.prototype.crearPerfil = function(button,ladda) {
	var obj = this;
	$.ajax({
				url : "/SistemaFE/perfiles/crearPerfil",
				type : "POST",
				data : $("#frmPerfil").serialize(),
				dataType : "json",
				beforeSend : function() {},
				success : function(data) {
					if (data.msg == "ok") {
						ladda.stop();
						swal({
							title : data.obs,
							text : '',
							type : 'success',
							confirmButtonColor : '#3085d6',
							confirmButtonText : 'Ok',
							confirmButtonClass : 'btn btn-success',
							buttonsStyling : false,
							allowOutsideClick : false
						}).then(function() {location.href = "/SistemaFE/perfiles/";})
					}else{
						bootbox.alert("<label class='text-danger'>ERROR : <br> Ocurrio un error al procesar la Solicitud</label>");
						ladda.stop();
					}
					
				},
				error : function(xhr, statusText, errorThrown) {
					// l.stop();
					ladda.stop();
					console.log("ocurrio un error");
				}
			});
	// })
}

Perfiles.prototype.updatePerfil = function() {
	console.log("click");
	$.ajax({
		url : "/SistemaFE/perfiles/updatePerfil/",
		type : "POST",
		data : $("#frmPerfil").serialize(),
		dataType : "json",
		beforeSend : function() {
			// $('#btnProccess').ladda().ladda('start');
		},
		success : function(data) {
			if (data.msg == "ok") {
				// Se añadio para el efecto de aviso que se guardo correctamente
				// Luis S.
				swal({
					title : 'Se actualizó correctamente el Perfil',
					text : '',
					type : 'success',
					confirmButtonColor : '#3085d6',
					confirmButtonText : 'Ok',
					confirmButtonClass : 'btn btn-success',
					buttonsStyling : false,
					allowOutsideClick : false
				}).then(function() {
					location.href = "/SistemaFE/perfiles/";
				})

			}else{
				bootbox.alert("<label class='text-danger'>ERROR : <br> Ocurrio un error al procesar la Solicitud</label>");
			}
		},
		error : function(xhr, statusText, errorThrown) {
			console.log("ocurrio un error");
		}
	});
}

Perfiles.prototype.reloadDataTableGrupoComer = function() {
	this.lstPerfiles.DataTable().ajax.url("/SistemaFE/perfiles/lista_perfiles"+'?'+$('#frm_filtrarPerfil').serialize()).load();
}

Perfiles.prototype.listarPerfiles = function () {
    var obj = this;
    obj.lstPerfiles.DataTable({
    	"sPaginationType": "simple_numbers",
    	  "scrollY" : "500px",
    	  "searching": false,
    	"scrollX": true,
    	"sScrollX": "100%",
    	"bScrollCollapse": true,
        "processing": false,
        "serverSide": true,
        "order": [],
        "colReorder": {
            "fixedColumnsLeft": 1,
            "fixedColumnsRight": 2
        },
        "pagingType": "numbers",
        "lengthMenu": [10,25,50,100],
        "pageLength": 10,
        "paging": true,
        "Paginate" : true,
        "ajax": {
            "url": "/SistemaFE/perfiles/lista_perfiles"+"?"+$('#frm_filtrarPerfil').serialize(),
            "type": "POST"
        },
        "columnDefs": [
        	{"targets": 0, "data": "perfilInd"},
            {"targets": 1, "data": "perfilName"},
            {"targets": 2, "data": "perfilDescr"},
            {
                "targets": 3,
                "data": "accion", "defaultContent": "",
                "className": "text-center",
                "orderable": false,
                "width": "100px",
                "render": function (data, type, full, meta) {
                	var estado="";
                    if(full.perfilIdEstado=="0"){
                    	 return 	"<div class='badge bg-red'>"+full.perfilDescrEstado+ "</div>";
                    } else {
                    	 return 	"<div class='badge' style='background-color: #15a01f;' >"+full.perfilDescrEstado+ "</div> ";
                    }
                   
                }
            },
            {
                "targets": 4,
                "data": "accion", "defaultContent": "",
                "className": "text-center",
                "orderable": false,
                "width": "100px",
                "render": function (data, type, full, meta) {
                	var estado="";
                	var tituloEstado="";
                	var boton="";
                    if(full.perfilIdEstado=="0"){
                    	estado="disabled";
                    	tituloEstado="Habilitar";
                    	boton="fa fa-check btn btn-success";
                    }else{
                    	tituloEstado="Inhabilitar";
                    	boton="fa fa-ban btn btn-danger";
                    }
                    
                    //https://fontawesome.com/v4.7.0/icons/
                    return 	"<button type='button' class='fa fa-edit editar btn btn-warning btn-sm btnEditarPerfil' title ='Modificar' attr-id='"+full.perfilID+"'" + "' attr-nombre='"+full.perfilName+"'" +
                    		""+estado+"> </button> " +
                    		"<button type='button' class='fa fa-user editar btn btn-info btn-sm btnAsigPerm'  title ='Asignar Permisos' attr-id='"+full.perfilID+"'" + "' attr-nombre='"+full.perfilName+"'" +
                            ""+estado+"> </button> " +
                            "<button type='button' class='"+boton+" btn-sm btnCambiarEstPerf'  title ='"+tituloEstado+"' attr-id='"+full.perfilID+"'" + "' attr-nombre='"+full.perfilName+"'" + 
                            "' attr-estado='"+full.perfilIdEstado+"'" + "> </button> ";
                }
            }

        ],
        "preDrawCallback": function (settings) {
            var api = new $.fn.dataTable.Api(settings);
            var pagination = $(this).closest('.dataTables_wrapper').find('.dataTables_paginate');
            pagination.toggle(api.page.info().pages > 1);
        },
        "drawCallback": function (settings) {
            $(".filterWrapping").append($(".filter-block"));
            $(".filterActive").append($("#filterActive"));
            $("#filterActive").removeClass("hidden");
            $(".filter-block").removeClass("hidden");
            $(".multiBtnWrapping").append($("#btnBar_multipleSelection"));
            $("#btnBar_multipleSelection").removeClass("hidden");
            $(this).closest('.dataTables_wrapper').removeClass("form-inline");
            $("input[id='select_all']").prop('checked', false);
            
        }
    });
    $("#lstEmpresa_filter").hide();
}


Perfiles.prototype.asigModPerfiles = function(button,ladda) {
	var obj = this;
	var listaId = $('#arbolSeleccionable').jstree("get_checked");
	
	var idPerfil = $('#perfil_idAP').val();
	var json ={
			"idPerfil":idPerfil,
			"modAsig": listaId
	};
	
	$.ajax({
				url : "/SistemaFE/perfiles/asigModPerfiles",
				type : "POST",
				data : JSON.stringify(json),
				dataType : "json",
				contentType: "application/json; charset=utf-8",
				beforeSend : function() {},
				success : function(data) {
					if (data.msg == "ok") {
						ladda.stop();
						swal({
							title : 'Se asignaron correctamente los Permisos',
							text : '',
							type : 'success',
							confirmButtonColor : '#3085d6',
							confirmButtonText : 'Ok',
							confirmButtonClass : 'btn btn-success',
							buttonsStyling : false,
							allowOutsideClick : false
						}).then(function() {location.href = "/SistemaFE/perfiles/";})
					}else{
						ladda.stop();
						bootbox.alert("<label class='text-danger'>ERROR : <br> Ocurrio un error al procesar la Solicitud</label>");
						
					}
					
				},
				error : function(xhr, statusText, errorThrown) {
					// l.stop();
					ladda.stop();
					console.log("ocurrio un error");
				}
			});
	// })
}

Perfiles.prototype.listPerfiles = function () {
	var id=$("#perfil_idAP").val();
	var obj=this;
	$.ajax({
		url : "/SistemaFE/perfiles/obtenerPerfiles/",
		type : "POST",
		data : {
			"id":id
		},
		dataType : "json",
		beforeSend : function() {
		},
		success : function(data) {
			$('#arbolSeleccionable').jstree({
				'plugins' : [
					"checkbox"
				],
                'core': {
                    'data': data.json
                },
                'checkbox': {
                	tie_selection : false 
                },
            }).on('loaded.jstree', function() {
            	$("#arbolSeleccionable").jstree('open_all');
            		obj.reloadArbol();
            }).on("check_node.jstree uncheck_node.jstree", function(e, data) {
            		        obj.reloadArbol();
            		});
			
			$('#arbol').jstree({
				'plugins' : [
					"types",
					"json_data",
					"dnd"
				],
                'core': {
                	'check_callback': true
                }
            }).on('create_node.jstree', function(e, data) {
                console.log('saved');
            }).on('loaded.jstree', function() {
            	$("#arbol").jstree('open_all');
            });
			$("#arbolSeleccionable").jstree('open_all');
			
			
			
		},
		error : function(xhr, statusText, errorThrown) {
			console.log("ocurrio un error");  
		}
	});
	
}

Perfiles.prototype.reloadArbol = function() {
	$('#arbol').jstree(true).settings.core.data = null;
	$('#arbol').jstree(true).refresh();
	var selecionados = $('#arbolSeleccionable').jstree("get_checked").sort();
	for(var y=0;y<selecionados.length;y++){
		var id=selecionados[y];
		var termino=true;
		var a =  $('#arbolSeleccionable').jstree(true).get_node(id);
		var listaNodos=[];
		listaNodos[0]=id;
		listaNodos.push.apply(listaNodos, a.parents);
		console.log("ids: "+listaNodos +" size: "+listaNodos.length);
		for(var t=listaNodos.length-1;t>=0;t--){
			console.log("nodo["+t+"] = "+listaNodos[t]);
			if(listaNodos[t]=='#'){
				console.log("Estas en el nodo #");
			}else{
				var aux =  $('#arbolSeleccionable').jstree(true).get_node(listaNodos[t]);
				console.log("id : "+listaNodos[t]+" padre: "+aux.parent);
				if($('#arbol').jstree(true).get_node(listaNodos[t])){
		          console.log("Existe el nodo no se creara");
		        }else{
		          console.log("no Existe el nodo se creara padre: "+aux.parent+" hijo :"+listaNodos[t]);
					$('#arbol').jstree().create_node(aux.parent, {
		               "id": aux.id,
		               "text": aux.text,
		                "icon": aux.icon,
						"state": "open"
		            }, "last", function() {
		            });
		        }
			}
		}
	}
	$("#arbol").jstree('open_all');
}

Perfiles.prototype.buscarUsers = function(button,ladda) {
	var obj = this;
	
	$( "#lista" ).empty();
	$.ajax({
				url : "/SistemaFE/perfiles/buscarUsers",
				type : "POST",
				data : {
					"perfil_id":$("#perfil_vmodule").val()
				},
				dataType : "json",
				beforeSend : function() {},
				success : function(data) {
					if (data.msg == "ok") {
						ladda.stop();
						if(data.usuarios!=undefined && data.usuarios.length>0){
							for(var y=0;y<data.usuarios.length;y++){
								//console.log(data.usuarios[y].user);
								$( "#lista" ).append( "<li> Usuario: "+data.usuarios[y].user+"</li>" );
							}
							
						}else{
							$( "#lista" ).append( "<li> No Existem usuarios</li>" );
						}
						/*swal({
							title : data.obs,
							text : '',
							type : 'success',
							confirmButtonColor : '#3085d6',
							confirmButtonText : 'Ok',
							confirmButtonClass : 'btn btn-success',
							buttonsStyling : false,
							allowOutsideClick : false
						}).then(function() {location.href = "/SistemaFE/perfiles/";})*/
					}else{
						bootbox.alert("<label class='text-danger'>ERROR : <br> Ocurrio un error al procesar la Solicitud</label>");
						ladda.stop();
					}
					
				},
				error : function(xhr, statusText, errorThrown) {
					// l.stop();
					ladda.stop();
					console.log("ocurrio un error");
				}
			});
	// })
}


